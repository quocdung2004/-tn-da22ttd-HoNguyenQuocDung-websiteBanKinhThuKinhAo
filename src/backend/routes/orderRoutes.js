...
const { verifyToken, verifyStaffOrAdmin, optionalVerifyToken, verifyAdminOnly, verifyShipperOnly } = require('../middleware/authMiddleware');
...
router.get('/shipper/assigned', verifyToken, verifyShipperOnly, orderController.getShipperAssignedOrders);

// Shipper gửi yêu cầu đối soát nộp tiền COD về công ty
router.post('/shipper/reconcile', verifyToken, verifyShipperOnly, orderController.requestReconciliation);

// Cập nhật kết quả giao hàng của Shipper (Thành công / Thất bại)
router.put('/shipper/:id/delivery', verifyToken, verifyShipperOnly, orderController.updateShipperDeliveryStatus);

// Shipper xác nhận đã thu hồi hàng vật lý đổi trả từ khách hàng
router.put('/shipper/:id/physical-return', verifyToken, verifyShipperOnly, orderController.confirmPhysicalReturn);

// Route sinh QR token bảo mật cho đơn hàng (Staff/Admin)
router.get('/:id/qr-token', verifyToken, verifyStaffOrAdmin, orderController.generateOrderQRToken);

// Route Shipper quét QR xem thông tin chi tiết đơn
router.get('/shipper/scan-order/:qrToken', verifyToken, verifyShipperOnly, orderController.scanOrder);

// Route Shipper bấm nhận đơn giao
router.post('/shipper/accept-order', verifyToken, verifyShipperOnly, orderController.acceptOrder);

// Route Shipper chốt đơn (Thành công/Thất bại)
router.post('/shipper/update-status', verifyToken, verifyShipperOnly, orderController.updateShipperStatus);
